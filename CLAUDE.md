...
Depois de abrir, o `rename()` **reposiciona a seleção**: pula o 1º caractere e seleciona a 1ª palavra do nome (equivale a `←` + `→` + `Alt+Shift+→`) — útil pros nomes com prefixo `[TAG]` (ex.: `[FAZENDO]`), deixando a tag selecionada pra digitar por cima. Eventos de teclado **sintéticos não movem o caret** (só eventos trusted do SO), então em vez de "mandar as setas" usamos a **Selection API** direto: `setSelectionRange(1, fim-da-1ª-palavra)`, com `fim` = `1 + match(/^\W*\w+/)`. O `<input>` aparece async (re-render do React), então re-tenta (`document.activeElement` é INPUT com `selectionStart`) até focar; testado ao vivo que a seleção **sobrevive** ao re-render.
