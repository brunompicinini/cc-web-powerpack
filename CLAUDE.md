...
**Template padrão:** sessão ainda sem nota salva abre com o template `# STATUS` / `# PRs` / `# CLICKUP` / `# LINKS` (const `TEMPLATE`, via `loadNote()` = `load(id) || TEMPLATE`). É só exibição — não grava no `localStorage` até o usuário editar (o `input`/`blur` salva o que estiver no editor). Nota vazia (nunca salva ou esvaziada) reexibe o template.
